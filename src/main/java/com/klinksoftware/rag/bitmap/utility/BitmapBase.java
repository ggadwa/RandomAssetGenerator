...
public class BitmapBase
{
    ...
    public void drawTriangleGradient(int x0, int y0, int x1, int y1, int x2, int y2, float f0, float f1, float f2, RagPoint n0, RagPoint n1, RagPoint n2, RagColor color, boolean doPerlinNoise, float colorFactorMin, float colorFactorAdd, boolean flipNormals, boolean doNormalNoise) {
        int x, y, lx, rx, ty, my, by, tyX, myX, byX, idx;
        float f, fSin, fLx, fRx, fTyX, fMyX, fByX;
        ...
        for (y = ty; y < my; y++) {
            ...
            for (x = lx; x <= rx; x++) {
                ...
                // color
                fSin = (float) Math.sin(((float) Math.PI * 0.5f) * ((float) (x - lx) / (float) ((rx - lx) + 1)));
                f = fLx + ((fRx - fLx) * fSin);
                ...
                // normal
                drawNormal.tween(nLx, nRx, fSin);
                if (flipNormals) {
                    drawNormal.x = -drawNormal.x;
                    drawNormal.y = -drawNormal.y;
                }
                if (doNormalNoise) {
                    drawNormal.x *= noiseNormals[idx];
                    drawNormal.y *= noiseNormals[idx + 1];
                    drawNormal.z *= noiseNormals[idx + 2];
                }
        ...
        for (y = my; y <= by; y++) {
            ...
            lx = myX + (int) ((float) ((byX - myX) * (y - my)) / (float) ((by - my) + 1));
            rx = tyX + (int) ((float) ((byX - tyX) * (y - ty)) / (float) (by - ty));
            fLx = fMyX + (((fByX - fMyX) * (float) (y - my)) / (float) ((by - my) + 1));
            fRx = fTyX + (((fByX - fTyX) * (float) (y - ty)) / (float) (by - ty));

            nLx.x = nMyX.x + (((nByX.x - nMyX.x) * (float) (y - my)) / (float) ((by - my) + 1));
            nLx.y = nMyX.y + (((nByX.y - nMyX.y) * (float) (y - my)) / (float) ((by - my) + 1));
            nLx.z = nMyX.z + (((nByX.z - nMyX.z) * (float) (y - my)) / (float) ((by - my) + 1));
    ...
    public void drawTriangleGradientWrap(int x0, int y0, int x1, int y1, int x2, int y2, float f0, float f1, float f2, RagPoint n0, RagPoint n1, RagPoint n2, RagColor color, boolean doPerlinNoise, float colorFactorMin, float colorFactorAdd, boolean flipNormals, boolean doNormalNoise) {
        int txtSize;

        // regular triangle
        drawTriangleGradient(x0, y0, x1, y1, x2, y2, f0, f1, f2, n0, n1, n2, color, doPerlinNoise, colorFactorMin, colorFactorAdd, flipNormals, doNormalNoise);

        // fix the wrap by drawing a triangle on the other side
        txtSize = textureSize - 1;
        if ((x0 < 0) || (x1 < 0) || (x2 < 0)) {
            drawTriangleGradient((txtSize + x0), y0, (txtSize + x1), y1, (txtSize + x2), y2, f0, f1, f2, n0, n1, n2, color, doPerlinNoise, colorFactorMin, colorFactorAdd, flipNormals, doNormalNoise);
        }
        if ((x0 >= textureSize) || (x1 >= textureSize) || (x2 >= textureSize)) {
            drawTriangleGradient((x0 - txtSize), y0, (x1 - txtSize), y1, (x2 - txtSize), y2, f0, f1, f2, n0, n1, n2, color, doPerlinNoise, colorFactorMin, colorFactorAdd, flipNormals, doNormalNoise);
        }
        if ((y0 < 0) || (y1 < 0) || (y2 < 0)) {
            drawTriangleGradient(x0, (txtSize + y0), x1, (txtSize + y1), x2, (txtSize + y2), f0, f1, f2, n0, n1, n2, color, doPerlinNoise, colorFactorMin, colorFactorAdd, flipNormals, doNormalNoise);
        }
        if ((y0 >= textureSize) || (y1 >= textureSize) || (y2 >= textureSize)) {
            drawTriangleGradient(x0, (y0 - txtSize), x1, (y1 - txtSize), x2, (y2 - txtSize), f0, f1, f2, n0, n1, n2, color, doPerlinNoise, colorFactorMin, colorFactorAdd, flipNormals, doNormalNoise);
        }
    }
    ...
    public ArrayList<GridPoint> createRandomGridVertexes(int count, float randomPercentage, int xStart, int yStart, int pixelSize) {
        int x, y, xOff, yOff;
        ...
        for (y = 0; y <= count; y++) {
            ...
            toPnt.x = ((count * gridSize) + fromPnt.x);
        ...
        for (x = 0; x <= count; x++) {
            ...
            toPnt.y = ((count * gridSize) + fromPnt.y);
        }
